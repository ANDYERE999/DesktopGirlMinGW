 ...
 * @author Misaki
 * 单例类
 * 本模块基于Qt6当中的Multimedia中的QMediaPlayer模块二次封装了一些用于音频播放的操作
 * 实现的功能
 * 1. 设定传入的音频文件路径
 * 2. 根据音频文件路径播放音频
 ...
 *  QMediaPlayer 提供了多个信号，用于通知状态变化：
        playbackStateChanged(QMediaPlayer::PlaybackState state)：播放状态变化时触发。
...
public:
     ...
     * @return QMediaPlayer::PlaybackState (Qt6中的新名称)
     */
...
private:
    ...
    QAudioOutput *audioOutput;  /// <!Qt6音频输出设备
};
